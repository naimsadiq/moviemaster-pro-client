...
const MovieCard = ({ movie }) => {
  const { title, rating, image_url, genres, _id } = movie;
  const genresString = genres.join(", ");

  const navigete = useNavigate();
  const handleMovieDetails = () => {
    navigete(`/movie-details/${_id}`);
  };
  return (
    <div className="hover:shadow-md w-60 pb-3 bg-[#ffffff] group relative transition rounded-b-sm col-span-1">
